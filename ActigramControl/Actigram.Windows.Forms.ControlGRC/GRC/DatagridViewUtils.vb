...
#Region "Gestion de la colonne de sélection"
    ...
    Public Sub ConfigColSel(ByVal dgv As DataGridView, ByVal colsel As DataGridViewCheckBoxColumn)
        AddHandler dgv.CellClick, AddressOf dg_SelectionHeaderClick
        AddHandler dgv.CellClick, AddressOf dg_SelectionCellClick
        colsel.Name = COL_SEL
    End Sub

    Public Sub dg_SelectionHeaderClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
    ...
    Public Sub dg_SelectionCellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        If e.RowIndex < 0 Then Exit Sub
        ...
        If TypeOf dgv.Columns(e.ColumnIndex) Is DataGridViewCheckBoxColumn Then
            ...
            Dim cell As DataGridViewCheckBoxCell = DirectCast(dgv.Rows(e.RowIndex).Cells(e.ColumnIndex), DataGridViewCheckBoxCell)
            cell.Value = Not CBool(cell.Value)
        End If
    End Sub

    Private Sub SetSelectedAll(ByVal dgv As DataGridView, ByVal selected As Boolean)
        dgv.SuspendLayout()
        For Each r As DataGridViewRow In dgv.Rows
    ...
    Public Function GetSelectedRowViews(ByVal dgv As DataGridView) As List(Of DataRowView)
        ...
        For Each r As DataGridViewRow In dgv.Rows
            If r.DataBoundItem IsNot Nothing Then
                If CBool(r.Cells(COL_SEL).Value) Then
                    res.Add(DirectCast(r.DataBoundItem, DataRowView))
                End If
            End If
        Next
        Return res
    ...
    Public Function GetSelectedRows(ByVal dgv As DataGridView) As List(Of DataRow)
        ...
        For Each r As DataGridViewRow In dgv.Rows
            If r.DataBoundItem IsNot Nothing Then
                If CBool(r.Cells(COL_SEL).Value) Then
                    res.Add(DirectCast(r.DataBoundItem, DataRowView).Row)
                End If
            End If
        Next
        Return res
