...
Module DatagridViewUtils
    ...
    Public Sub dg_ZoomTextBoxCells(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs)
        ...
        If Cast(Of DataGridView)(sender).ReadOnly Then Exit Sub
        If Cast(Of DataGridView)(sender).Columns(e.ColumnIndex).ReadOnly Then Exit Sub
        If Cast(Of DataGridView)(sender).Rows(e.RowIndex).Cells(e.ColumnIndex).ReadOnly Then Exit Sub
...
#Region "Gestion de la colonne de sélection"
    Private Const COL_SEL As String = "ColSel"

    Public Sub ConfigColSel(ByVal dgv As DataGridView, ByVal colsel As DataGridViewCheckBoxColumn, Optional ByVal SelectionChangedHandler As EventHandler = Nothing)
        AddHandler dgv.CellClick, AddressOf dg_SelectionHeaderClick
        AddHandler dgv.CellClick, AddressOf dg_SelectionCellClick
        colsel.Name = COL_SEL
        colsel.Tag = SelectionChangedHandler
        Dim ctx As ContextMenuStrip
        If dgv.ContextMenuStrip IsNot Nothing Then
            ctx = dgv.ContextMenuStrip
        Else
            ctx = New ContextMenuStrip
            dgv.ContextMenuStrip = ctx
        End If
        If ctx.Items.Count > 0 Then ctx.Items.Add(New ToolStripSeparator)
        With ctx.Items.Add("Sélectionner", My.Resources.checkAll)
            .ImageScaling = ToolStripItemImageScaling.None
            .Tag = dgv
            AddHandler .Click, AddressOf dg_CheckSelected
        End With
        With ctx.Items.Add("Désélectionner", My.Resources.uncheck)
            .ImageScaling = ToolStripItemImageScaling.None
            .Tag = dgv
            AddHandler .Click, AddressOf dg_UncheckSelected
        End With
    End Sub

    Private Sub dg_SelectionHeaderClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        Dim dgv As DataGridView = CType(sender, DataGridView)
        If e.RowIndex < 0 AndAlso dgv.Columns(e.ColumnIndex).Name = COL_SEL Then
            Dim allDeselected As Boolean = True
            For Each r As DataGridViewRow In dgv.Rows
                If CBool(r.Cells(COL_SEL).Value) Then
                    allDeselected = False
                    Exit For
                End If
            Next
            SetSelectedAll(dgv, allDeselected)
            If dgv.Columns(e.ColumnIndex).Tag IsNot Nothing Then
                If TypeOf dgv.Columns(e.ColumnIndex).Tag Is EventHandler Then
                    CType(dgv.Columns(e.ColumnIndex).Tag, EventHandler).Invoke(sender, EventArgs.Empty)
                End If
            End If
        End If
    End Sub

    Private Sub dg_CheckSelected(ByVal sender As Object, ByVal e As EventArgs)
        ChangeCheckSelected(CType(sender, ToolStripItem), True)
    End Sub

    Private Sub dg_UncheckSelected(ByVal sender As Object, ByVal e As EventArgs)
        ChangeCheckSelected(CType(sender, ToolStripItem), False)
    End Sub

    Private Sub ChangeCheckSelected(ByVal i As ToolStripItem, ByVal value As Boolean)
        If i.Tag Is Nothing Then Exit Sub
        If Not TypeOf i.Tag Is DataGridView Then Exit Sub
        Dim dgv As DataGridView = CType(i.Tag, DataGridView)
        dgv.SuspendLayout()
    ...
    Private Sub dg_SelectionCellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        If e.RowIndex < 0 Then Exit Sub
        If e.ColumnIndex < 0 Then Exit Sub
        Dim dgv As DataGridView = CType(sender, DataGridView)
        If TypeOf dgv.Columns(e.ColumnIndex) Is DataGridViewCheckBoxColumn Then
            If dgv.Columns(e.ColumnIndex).Name <> COL_SEL Then Exit Sub
            Dim cell As DataGridViewCheckBoxCell = Cast(Of DataGridViewCheckBoxCell)(dgv.Rows(e.RowIndex).Cells(e.ColumnIndex))
    ...
    Public Sub SetSelectedAll(ByVal dgv As DataGridView, ByVal selected As Boolean)
        dgv.SuspendLayout()
        For Each r As DataGridViewRow In dgv.Rows
            r.Cells(COL_SEL).Value = selected
        Next
        dgv.ResumeLayout()
    End Sub

    Public Function GetSelectedRowViews(ByVal dgv As DataGridView) As List(Of DataRowView)
        Dim res As New List(Of DataRowView)
        'Les lignes cochées
        For Each r As DataGridViewRow In dgv.Rows
            If r.DataBoundItem IsNot Nothing Then
                If CBool(r.Cells(COL_SEL).Value) Then
                    res.Add(Cast(Of DataRowView)(r.DataBoundItem))
                End If
            End If
        Next
        'S'il n'y en a pas, les lignes surlignées
        If res.Count = 0 Then
            For Each r As DataGridViewRow In dgv.SelectedRows
                If r.DataBoundItem IsNot Nothing Then
                    res.Add(Cast(Of DataRowView)(r.DataBoundItem))
    ...
    Public Function GetSelectedRows(ByVal dgv As DataGridView) As List(Of DataRow)
        Dim res As New List(Of DataRow)
        'Les lignes cochées
        For Each r As DataGridViewRow In dgv.Rows
            If r.DataBoundItem IsNot Nothing Then
                If CBool(r.Cells(COL_SEL).Value) Then
                    res.Add(Cast(Of DataRowView)(r.DataBoundItem).Row)
                End If
            End If
        Next
        'S'il n'y en a pas, les lignes surlignées
...
#End Region
    ...
    Public Sub dg_DataError(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs)
        ...
        ElseIf Not TypeOf e.Exception Is UserCancelledException AndAlso Not TypeOf e.Exception Is IndexOutOfRangeException Then
            MsgBox(e.Exception.Message, MsgBoxStyle.Exclamation, "Erreur")
        End If
        e.ThrowException = False
    End Sub

    Public Sub ApplyStyle(ByVal dgv As DataGridView, Optional ByVal readOnlyGrid As Boolean = True, Optional ByVal useSystemFont As Boolean = True)
        With dgv
    ...
    Private Function alternateCellStyle() As System.Windows.Forms.DataGridViewCellStyle
        ...
        With style
            ...
            .SelectionBackColor = System.Drawing.Color.FromArgb(255, 240, 108)
            .SelectionForeColor = Color.Black
        End With
        Return style
    End Function

    Private Function cellStyle() As System.Windows.Forms.DataGridViewCellStyle
        Dim style As System.Windows.Forms.DataGridViewCellStyle = New System.Windows.Forms.DataGridViewCellStyle
        With style
    ...
    Protected Overrides Function ProcessCmdKey(ByRef msg As System.Windows.Forms.Message, ByVal keyData As System.Windows.Forms.Keys) As Boolean
        ...
        If key = Keys.Delete Then
            If Not Me.ReadOnly Then
                ...
                MyBase.CurrentCell.Value = DBNull.Value
            End If
            Return True
        Else
