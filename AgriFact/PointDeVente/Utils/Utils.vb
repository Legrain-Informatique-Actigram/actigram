...
Module DecimalUtils
    ...
    Public Sub ConfigPercentControl(ByVal c As Control, Optional ByVal decimals As Integer = 2, Optional ByVal SaisieEntiere As Boolean = True)
        If c.DataBindings("Text") IsNot Nothing Then
            With c.DataBindings("Text")
                If SaisieEntiere Then
                    .FormatString = String.Format("#0{0}{1}'%'", IIf(decimals > 0, ".", ""), New String("0"c, decimals))
                Else
    ...
    Public Function IsValidForNumberInput(ByVal c As Char) As Boolean
        Return Char.IsDigit(c) OrElse Char.IsControl(c) OrElse Char.IsPunctuation(c) OrElse c = "+" OrElse c = Application.CurrentCulture.NumberFormat.CurrencySymbol
    End Function
End Module
