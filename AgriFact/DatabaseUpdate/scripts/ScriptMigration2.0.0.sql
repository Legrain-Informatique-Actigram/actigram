...
from [Niveau2] inner join Niveau2 as n on n.nNiveau1=0 AND n.[Table]=[Niveau2].[Table] AND n.Champs='Secteur'
where [Niveau2].nNiveau1=0 AND [Niveau2].Champs='nTarif'
AND [Niveau2].[Table] in ('VDevis','VBonCommande','VBonLivraison','VFacture','AFacture','ABonReception')

update [Niveau2] set Libelle='U2=AxU1' WHERE [Table]='Produit' AND Champs='CoefU2'
update [Niveau2] set LW=60 WHERE [Table]='Produit' AND Champs='CoefU2'
update [Niveau2] set LW=60 WHERE [Table]='Produit' AND Champs='Unite2'
update [Niveau2] set LW=NULL WHERE [Table]='Produit' AND Champs='Unite1'
update [Niveau2] set LW=NULL,TW=NULL WHERE [Table]='Produit' AND Champs='Inactif'
update [Niveau2] set LW=NULL,TW=230 WHERE [Table]='Produit' AND Champs='DecompteAuto'
update [Niveau2] set LW=NULL,TW=NULL WHERE [Table]='Produit' AND Champs='GestionStock'
update [Niveau2] set LW=NULL,TW=230 WHERE [Table]='Produit' AND Champs='ProduitCompose'
update [Niveau2] set LX=NULL WHERE [Table]='Produit' AND GroupeDispo='PV'

update [Niveau2] set TW=170 WHERE [Table]='Produit' AND Champs='ProduitVente'
update [Niveau2] set TW=170 WHERE [Table]='Produit' AND Champs='ProduitAchat'
update [Niveau2] set LW=85 WHERE [Table]='Produit' AND Champs='PrixVHT'
update [Niveau2] set LW=85 WHERE [Table]='Produit' AND Champs='PrixVTTC'
update [Niveau2] set TW=80, LW=85 WHERE [Table]='Produit' AND Champs='PrixAHT'
update [Niveau2] set TW=80, LW=85 WHERE [Table]='Produit' AND Champs='PrixATTC'
update [Niveau2] set TW=80, LW=85 WHERE [Table]='Produit' AND Champs='CoefAV'

update Niveau2 set Libelle='Mode facturation' where [Table]='Produit' and Champs='TypeFacturation'
update Niveau2 set LX=NULL, LH=40 where [Table]='Produit' and Champs='NCompteV'
update Niveau2 set LX=1,LH=40 where [Table]='Produit' and Champs='NActiviteV'
update Niveau2 set LX=NULL,LH=40 where [Table]='Produit' and Champs='NCompteA'
update Niveau2 set LX=1,LH=40 where [Table]='Produit' and Champs='NActiviteA'

update Niveau2 set TW=NULL where [Table]='Entreprise' and Champs='Fournisseur'
update Niveau2 set TW=NULL where [Table]='Entreprise' and Champs='Client'

update Niveau2 set TW=85,LX=NULL, LH=40 where [Table]='Entreprise' and Champs='NCompteF'
update Niveau2 set TW=65,LW=88,LX=1,LH=40 where [Table]='Entreprise' and Champs='NActiviteF'
update Niveau2 set TW=85,LX=NULL,LH=40 where [Table]='Entreprise' and Champs='NCompteC'
update Niveau2 set TW=65,LW=88,LX=1,LH=40 where [Table]='Entreprise' and Champs='NActiviteC'

update [Niveau2] set LW=NULL,TW=80 WHERE [Table]='Evenement' AND Champs='DateEv' 

delete from Niveau2 where [Table]='Lot' And Champs in ('Famille1','Termine','QtDepart')
update Niveau2 set IsObligatoire=1 where [Table]='Lot' And Champs='nLot'
GO
grant Execute ON CalculInventaire to Utilisateurs 
GO
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[LotsTermines]') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
BEGIN
CREATE TABLE [dbo].[LotsTermines](
	[nLot] [nvarchar](50) NOT NULL,
	[CodeProduit] [nvarchar](50) NOT NULL,
 CONSTRAINT [PK_LotsTermines] PRIMARY KEY CLUSTERED 
(
	[nLot] ASC,
	[CodeProduit] ASC
))
END
GO
GRANT SELECT,INSERT,UPDATE,DELETE ON [dbo].[LotsTermines] TO Utilisateurs
GO
...
WHERE TypeMouvement IN ('Entrées','Fabrication','Inventaire','Conditionnement','Transfert dépôt','Régul stock')
...
DROP TABLE #tmp2
GO
ALTER PROCEDURE [dbo].[CalculInventaire](@dtDeb datetime, @dtFin datetime,@depot nvarchar(50),@gestionLot bit=1)
AS

/*declare @dtDeb datetime, @dtFin datetime,@depot nvarchar(50),@gestionLot bit
set @dtDeb='09/10/2009'
set @dtFin='09/10/2009'
set @depot=''
set @gestionLot=0*/

create table #tmpInv(Depot nvarchar(50),	nLot nvarchar(50),	Famille nvarchar(255),	CodeProduit nvarchar(255),	
					Libelle nvarchar(255),	SeuilStock  decimal(18,3),ProduitAchat bit,ProduitVente bit,	LibUnite1 nvarchar(2),	LibUnite2 nvarchar(2),	
					Depart decimal(18,3),	Entrée decimal(18,3),	Sortie decimal(18,3),	EnCommande decimal(18,3),
					DepartU2 decimal(18,3),	EntréeU2 decimal(18,3),	SortieU2 decimal(18,3),	EnCommandeU2 decimal(18,3),LotTermine bit)

insert into #tmpInv
exec calculetatstock @dtDeb,@dtFin,@depot,@gestionLot

select f.famille,p.codeproduit,p.libelle,unite1 as libunite1,unite2 as libunite2,u1u2independant,coefu2,
Depot,nlot, 
'QteU1Depart'= Depart+Entrée+Sortie,
'QteU1'= Depart+Entrée+Sortie,
Depart as DepartU1,DepartU2,Entrée as EntréeU1,EntréeU2,Sortie as SortieU1,SortieU2,
'QteU2Depart' = case when U1U2Independant=0 and CoefU2 is not null then (Depart+Entrée+Sortie)*CoefU2 else DepartU2+EntréeU2+SortieU2 end,
'QteU2' = case when U1U2Independant=0 and CoefU2 is not null then (Depart+Entrée+Sortie)*CoefU2 else DepartU2+EntréeU2+SortieU2 end
from produit p
inner join famille f on p.Famille1=f.nFamille
left join #tmpInv t on p.CodeProduit=t.CodeProduit
where gestionstock=1

drop table #tmpInv
GO
--
--	GESTION DES INFOS COMPTA DE PRODUIT A L'EXPORT
--
ALTER TABLE dbo.Produit ADD
	NCompteX nvarchar(8) NULL,
	NActiviteX nvarchar(4) NULL
GO
update produit set ncomptex=ncomptev,nactivitex=nactivitev
GO
if not exists(select * from Niveau2 where [table]='produit' and Champs='NCompteX') begin
	insert into Niveau2
	select [nNiveau1],nNiveau2+10,[Table],'NCompteX','N°Compte Export',[MaxLength],[ListeChoix],[ListeChoixMultipleTable],[ListeChoixMultipleChamps],[ListeChoixMultiple],
			[TableListeChoixType],[TableListeChoix],[TableListeChoixSelectChamps],[TableListeChoixAfficheChamps],[TableListeChoixTri],[FiltrePlus],[FiltreType],
			[StopSaisi],[NiveauAutorisation],[NpAfficherFormulaire],[AfficherRecherche],[AfficherFormulaireRecherche],[MultiLine],[HauteurMultiline],[ChampsConditionDispo],
			[GroupeDispo],[ChampsValeurDispo],[OuvrirFormulaire],[NpAfficherPrLien],[IsObligatoire],[IsMail],[IsAdresseWeb],[IsTime],[FindPoints],[IsRTF],
			[LX],[LY],[LW],[LH],[TX],[TY],[TW],[TH],[NbLigne] 
	from niveau2 where [table]='produit' and Champs='NCompteV'
end
if not exists(select * from Niveau2 where [table]='produit' and Champs='NActiviteX') begin
	insert into Niveau2
	select [nNiveau1],nNiveau2+10,[Table],'NActiviteX','N°Activité Export',[MaxLength],[ListeChoix],[ListeChoixMultipleTable],[ListeChoixMultipleChamps],[ListeChoixMultiple],
			[TableListeChoixType],[TableListeChoix],[TableListeChoixSelectChamps],[TableListeChoixAfficheChamps],[TableListeChoixTri],[FiltrePlus],[FiltreType],
			[StopSaisi],[NiveauAutorisation],[NpAfficherFormulaire],[AfficherRecherche],[AfficherFormulaireRecherche],[MultiLine],[HauteurMultiline],[ChampsConditionDispo],
			[GroupeDispo],[ChampsValeurDispo],[OuvrirFormulaire],[NpAfficherPrLien],[IsObligatoire],[IsMail],[IsAdresseWeb],[IsTime],[FindPoints],[IsRTF],
			[LX],[LY],[LW],[LH],[TX],[TY],[TW],[TH],[NbLigne] 
	from niveau2 where [table]='produit' and Champs='NActiviteV'
end
GO

if not exists(select * from syscolumns c inner join sysobjects o on c.id=o.id where c.name='IsExport' and o.name='Entreprise') 
BEGIN
	ALTER TABLE Entreprise ADD IsExport BIT DEFAULT 0
END
GO

if exists(select * from syscolumns c inner join sysobjects o on c.id=o.id where c.name='IsExport' and o.name='Entreprise') 
BEGIN
	UPDATE entreprise SET IsExport=0
END
GO
