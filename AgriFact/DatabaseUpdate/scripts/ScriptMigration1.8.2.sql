...
alter table vdevis add Depot nvarchar(50) null
if not exists (select * from syscolumns c inner join sysobjects t on c.id=t.id where c.name='Depot' and t.name='VBonCommande')
alter table vboncommande add Depot nvarchar(50) null
if not exists (select * from syscolumns c inner join sysobjects t on c.id=t.id where c.name='Depot' and t.name='VBonLivraison')
alter table vbonlivraison add Depot nvarchar(50) null
if not exists (select * from syscolumns c inner join sysobjects t on c.id=t.id where c.name='Depot' and t.name='VFacture')
alter table vfacture add Depot nvarchar(50) null
GO
IF EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[CalculEtatStock]') AND type in (N'P', N'PC'))
BEGIN
	DROP PROC [dbo].[CalculEtatStock]
END
GO
CREATE PROC [dbo].[CalculEtatStock](@dtDeb datetime, @dtFin datetime,@depot nvarchar(50),@gestionLot bit=1,@dtInit datetime='01/01/2000',@debug bit=0) AS
...
SELECT Depot,nLot,f.Famille,t.CodeProduit,p.Libelle,p.SeuilStock,p.ProduitAchat,p.ProduitVente,LibUnite1,LibUnite2,
...
GROUP BY f.Famille,t.CodeProduit,Depot,nLot,p.Libelle,p.SeuilStock,p.ProduitAchat,p.ProduitVente,LibUnite1,LibUnite2
ORDER BY f.Famille,t.CodeProduit,Depot,nLot

DROP TABLE #tmp
DROP TABLE #tmp2
GO
GRANT EXECUTE ON [CalculEtatStock] TO Utilisateurs
GO
--Ajout des champs Depot dans l'interface
CREATE TABLE #tmpI (nomTable varchar(50))
INSERT INTO #tmpI VALUES('VBonCommande')
INSERT INTO #tmpI VALUES('VBonLivraison')
INSERT INTO #tmpI VALUES('VFacture')
INSERT INTO #tmpI VALUES('ABonReception')
INSERT INTO #tmpI VALUES('AFacture')

declare @table varchar(50)
DECLARE curs CURSOR FOR SELECT nomTable FROM #tmpI
open curs
FETCH NEXT FROM curs INTO @table
WHILE @@FETCH_STATUS=0 BEGIN
	IF NOT EXISTS(SELECT * FROM Niveau2 WHERE nNiveau1=0 AND [Table]=@table AND Champs='Depot') BEGIN
		INSERT INTO Niveau2 (nNiveau1,nNiveau2,[Table],Champs,Libelle,ListeChoix,StopSaisi,NpAfficherFormulaire,AfficherRecherche,AfficherFormulaireRecherche)
		SELECT 0,MAX(nNiveau2)+1,@table,'Depot',N'Dépôt','ListeDepots',0,0,1,1
		FROM Niveau2 WHERE nNiveau1=0
	END
	FETCH NEXT FROM curs INTO @table
END
CLOSE curs
