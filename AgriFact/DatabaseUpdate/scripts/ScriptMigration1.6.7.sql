...
UPDATE Mouvement SET TypeMouvement='Transfert dépôt' WHERE TypeMouvement='Mouvement'

GO

-- Ajouter un type "Régul stock" (qui agit comme un conditionnement)
IF NOT EXISTS(SELECT * FROM [ListeChoix] WHERE NomChoix='ListeTypeMouvement' AND Valeur='Régul stock') BEGIN
	INSERT INTO [ListeChoix]([NomChoix], [nOrdreValeur], [Valeur])
	SELECT 'ListeTypeMouvement', MAX(nOrdreValeur)+10,'Régul stock' FROM ListeChoix WHERE NomChoix='ListeTypeMouvement'
END

GO

-- MODIF DE LA PROC STOCK
