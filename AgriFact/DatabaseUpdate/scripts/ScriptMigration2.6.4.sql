...
SELECT Depot,ListeLots.nLot,f.Famille,ListeLots.CodeProduit,p.Libelle,CASE WHEN p.SeuilStock IS NULL THEN 0 ELSE p.SeuilStock END AS SeuilStock,p.ProduitAchat,p.ProduitVente,LibUnite1,LibUnite2,
...
FROM (	SELECT Lot.nLot, LotProduit.CodeProduit
		...
		WHERE (LotsTermines.nLot IS NULL) AND (LotProduit.CodeProduit=@CodeProduit)
		GROUP BY Lot.nLot, LotProduit.CodeProduit) AS ListeLots LEFT JOIN 
...
GROUP BY f.Famille,t.CodeProduit,Depot,t.nLot,p.Libelle,p.SeuilStock,p.ProduitAchat,p.ProduitVente,LibUnite1,LibUnite2,ListeLots.nLot,ListeLots.CodeProduit
